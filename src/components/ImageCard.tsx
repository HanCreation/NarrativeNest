...
const ImageCard: React.FC<ImageCardProps> = ({ basePrompt }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Cast result to unknown first, then to our expected type
      const typedResult = (result as unknown) as ImageResult;
      
      if (typedResult.data && Array.isArray(typedResult.data) && typedResult.data[0]?.url) {
        setImage(typedResult.data[0].url);
      } else {
    ...
    <div className="p-4 bg-[#5C352B]/30 backdrop-blur-lg rounded-lg shadow-md relative w-72">
      <div className="w-full h-48 bg-gray-300 rounded-t-lg mb-2 flex items-center justify-center overflow-hidden">
        {loading ? (
          ...
          <img src={image} alt="Generated image" className="object-cover h-full w-full" />
        ) : (
